...
#define CLI_CMDLEN	4096		/* CLI single command length */
...
typedef int (*cli_clear_func)(void *ctx, int when, void *val);
...
typedef struct cli_command {
	char		cmd[CLI_CMDLEN];	/* the command */
	char		desc[CLI_COMMLEN];	/* the command desc */
	uint32		flags;			/* flags */
