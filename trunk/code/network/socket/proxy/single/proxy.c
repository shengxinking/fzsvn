...
#ifdef	_PROXY_DEBUG
#define	DBG(fmt, args...)	printf("proxy: "fmt, ##args)
#define FLOW(fmt, args...)	printf(fmt, ##args)
#else
#define	DBG(fmt, args...)
...
_proxy_init(proxy_t *ctx)
{
	...
	if (ctx->listenfd < 0) {
		ERR("create listen fd error\n");
		return -1;
	}
	sk_set_nonblock(ctx->listenfd, 1);
	DBG("create listen fd %d\n", ctx->listenfd);
	...
	if (ctx->capacity > 256) {
		rlim.rlim_cur = 4 * ctx->capacity + 10;
		rlim.rlim_max = 4 * ctx->capacity + 10;
...
_proxy_accept_client(proxy_t *ctx)
{
	...
	clifd = sk_accept(ctx->listenfd, &cliaddr);
	if (clifd < 0) {
...
_proxy_recv_data(proxy_t *ctx)
{
	...
	for (i = 0; i < nfds; i++) {
		 ...
		 */
		if (e->data.fd == ctx->listenfd) {
			if (ctx->stat.nliveconn < ctx->capacity) {
				_proxy_accept_client(ctx);
			}
			else {
				FLOW("too many connection %lu, max %u\n",
				     ctx->stat.nliveconn, ctx->capacity);
			}
		}
		/* recv data */
		else {
	   ...
	   int max, int use_splice, int use_nb_splice)
{
	...
	ctx.nrsaddr = nrsaddr;
	ctx.capacity = max;
	ctx.use_splice = use_splice;
	ctx.use_nb_splice = use_nb_splice;
	...
	printf("start proxy %s, capacity %d\n", 
	       ip_port_to_str(svraddr, buf, sizeof(buf)), max);
	printf("real servers(%d):\n", nrsaddr);
	for (i = 0; i < nrsaddr; i++) {
		printf("\t%d: %s\n", i, 
