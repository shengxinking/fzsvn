...
_send_init(thread_t *info)
{
	...
	DBG("send(%d) create epoll fd %d\n", info->index, sinfo->epfd);
...
_send_free_session(thread_t *info, session_t *s)
{
	...
	if (s->clifd > 0) {
		recv_del_fd(info->index, s->id, s->clifd);
	}

	if (s->svrfd > 0 && !(s->flags & SESSION_SVRWAIT)) {
		recv_del_fd(info->index, s->id, s->svrfd);
	}
	...
	DBG("send(%d) id %u free session, refcnt %d\n", 
...
_send_get_pkt(thread_t *info)
{
	...
	if (sinfo->inq.len > 0) {
		DBG("send(%d) get %d packet from inq\n", 
		    info->index, sinfo->inq.len);
...
_send_get_svrfd(thread_t *info, session_t *s)
{
	...
	DBG("send(%d) id %u get svrfd %d\n", info->index, s->id, svrfd);
	...
	DBG("send(%d) id %d add svrfd %d to send epoll\n", 
	    info->index, s->id, svrfd);
...
_send_check_svrfd(thread_t *info)
{
	...
	for (i = 0; i < n; i++) {
		...
		DBG("send(%d) id %u svrfd %d connect success\n", 
		    info->index, id, fd);

		/* notify recv thread */
		recv_add_fd(info->index, id, fd);
	}
...
_send_check_pkt(thread_t *info, packet_t *pkt, session_t *s)
{
	...
	if (PACKET_IS_CLI2SVR(pkt->flags)) {
		...
		if (s->flags & SESSION_SVRWAIT) {
			DBG("send(%d) id %u need wait svrfd %d\n", 
			    info->index, pkt->sid, s->svrfd);
			sesspool_put(g_proxy.sesspools[info->index], s);
			return 1;
		}
	}
...
_send_pkt(thread_t *info)
{
	...
	while (pkt) {
		...
		/* find session */
		s = sesspool_find(g_proxy.sesspools[info->index], pkt->sid);
		if (!s) {
		...
		if (n ==  (pkt->len - pkt->sendpos)) {
			DBG("send(%d) id %u send %d bytes finished\n", 
			    info->index, pkt->sid, m);
			if (PACKET_IS_CLI2SVR(pkt->flags)) {
		...
		/* not send all data, need block */
			...
			DBG("send(%d) id %u send packet blocked(%d:%d)\n", 
			    info->index, pkt->sid, m, n);
			pkt->sendpos += n;
			pktqueue_in(&pq, pkt);
			if (PACKET_IS_CLI2SVR(pkt->flags))
				s->flags |= SESSION_SVRBLOCK;
