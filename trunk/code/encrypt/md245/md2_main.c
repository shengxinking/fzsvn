...
static int _do_loop(void)
{
	...
	unsigned char digest[MD2_DIGEST_LENGTH + 1];
	int n = 0;
	
	do {
		n = read(_g_infd, buf, _BUFSIZ);
		if (n > 0)
			MD2_Update(&_g_md2ctx, buf, n);
	} while (n > 0);

	MD2_Final(digest, &_g_md2ctx);
	digest[MD2_DIGEST_LENGTH] = 0;

	if (_g_outfd >= 0) {
		write(_g_outfd, digest, MD2_DIGEST_LENGTH);
		close(_g_outfd);
		_g_outfd = -1;
	}
	else {
		int i;
		printf("MD2(%s)= ", _g_infile);
