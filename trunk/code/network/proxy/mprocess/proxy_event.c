...
_proxy_initiate(proxy_t *ctx)
{
	...
	ctx->httpfd = sk_tcp_server(&ctx->svraddr);
	if (ctx->listenfd < 0) {
	...
	if (ctx->maxsess > 512) {
		rlim.rlim_cur = 3 * ctx->maxsess;
		rlim.rlim_max = 3 * ctx->maxsess;
	...
	ctx->epctx = ep_event_create(ctx->maxsess);
	if (!ctx->epctx) {
	   ...
	   int max, int use_splice, int use_nb_splice)
{
	...
	printf("start proxy %s, listen fd %d, capacity %d\n", 
