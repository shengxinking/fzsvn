...
#ifndef FZ_HTTP_INFO_H
#define FZ_HTTP_INFO_H
...
typedef struct http_buffer {
	u_int16_t	is_full : 1;	/* the buffer is full */
	u_int16_t	in_use: 1;	/* the buffer is in use */
	u_int16_t	capacity;	/* buffer capacity */
	u_int16_t	start;		/* free space start position */
	u_int16_t	end;		/* the end position */
...
typedef struct http_string {
	...
	u_int16_t	ori_start;	/* start position in origin packet */
...
/**
 *	All http header parameters.
 */
typedef struct http_headpara {
	http_buffer_t	buffer;		/* buffer for store header parameter */
	...
	_http_arg_t	url[HTTP_MAX_URLARG]; /* URL arguments */
	http_string_t	tmp;

	/* the following is need remove when response is coming */
	_http_cookie_t	cookies[HTTP_MAX_COOKIE];	/* cookies */
...
typedef struct http_bodyarg {
	_http_arg_t	args[HTTP_MAX_BODYARG];/* body arguments */
...
#define	HTTP_MAX_FILESIG	512	/* first 512 bytes as signature */ 
...
typedef struct http_info {
	...
	http_headpara_t	headpara;	/* header parameter */
	http_bodyarg_t	bodyargs;	/* body args */
