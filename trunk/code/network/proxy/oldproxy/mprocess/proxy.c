...
_proxy_init(proxy_t *ctx)
{
	...
	if (ctx->maxsess > 256) {
		rlim.rlim_cur = 4 * ctx->maxsess + 20;
		rlim.rlim_max = 4 * ctx->maxsess + 20;
	...
	ctx->sessions = session_table_alloc(ctx->maxsess, 0);
	if (!ctx->sessions) {
	...
	for (i = 0; i < ctx->npolicy; i++) {
		pl = &ctx->policies[i];
		pl->httpfd = sk_tcp_server(&ctx->svraddr, 1);
		if (pl->httpfd < 0) {
		...
		pair[1] = -(i);
		e.events = EPOLLIN;
		if (epoll_ctl(ctx->recv_epfd, EPOLL_CTL_ADD, pl->httpfd, &e)) {
...
_proxy_accept_client(proxy_t *ctx)
{
	...
	for (i = 0; i < 3; i++) {
		clifd = sk_tcp_accept(ctx->httpfd, &cliaddr);
		if (clifd < 0) {
		...
		FLOW("ssn[%d] accept client %d %s->%s\n", s->id, clifd, 
				...
				ip_port_to_str(&ctx->svraddr, ipstr2, IP_STR_LEN));
	}
