...
static cproc_t		_g_workers[MAX_WORKER];	/* child processes */
static int		_g_nworker;	/* the number of child process */
static int		_g_stop;	/* the stop signal */
...
_parse_cmd(int argc, char **argv)
{
	...
	while ( (c = getopt(argc, argv, optstr)) != -1) {
		...
		case 'w':
			_g_nworker = atoi(optarg);
			if (_g_nworker < 1 || _g_nworker > MAX_WORKER) {
	...
	if (_g_arg.max == 0) 
		_g_arg.max = 100 / _g_nworker;

	if (_g_arg.use_splice)
		_g_arg.maxfd = _g_arg.max * 4 + _g_arg.npolicy + 10;
	else 
		_g_arg.maxfd = _g_arg.max * 2 + _g_arg.npolicy + 10;
...
static int 
_create_workers(void)
{
	int i;

	for (i = 0; i < _g_nworker; i++) {
		_g_arg.index = i;
		cproc_create(&_g_workers[i], &_g_arg);
	}
...
static int 
_destroy_workers(void)
{
	int i;

	for (i = 0; i < _g_nworker; i++) {
		if (_g_workers[i].pid > 0)
			cproc_destroy(&_g_workers[i]);
	}
