...
_recv_free_session(thread_t *info, session_t *s)
{
	...
	if (s->clifd > 0) {
		...
		if (epoll_ctl(rinfo->epfd, EPOLL_CTL_DEL, s->clifd, NULL)) {
			...
			DBG("id %u delete clifd %d from recv epoll\n", 
			    id, s->clifd);
			s->refcnt--;
		}
	}
	if (s->svrfd > 0) {
		...
		if (epoll_ctl(rinfo->epfd, EPOLL_CTL_DEL, s->svrfd, NULL)) {
			...
			DBG("id %u delete svrfd %d from recv epoll\n", 
			    id, s->svrfd);
			s->refcnt--;
		}
	}
	...
	work_del_id(index, id);

//	sesspool_print(g_proxy.sesspool);
...
_recv_add_fd(thread_t *info)
{
	...
	for (i = 0; i < size; i++) {
		...
		/* check session */
		s = sesspool_find(g_proxy.sesspool, pairs[i].id);
		if (!s) {
		...
		DBG("id %u fd %d add to recv epoll\n", 
		    pairs[i].id, pairs[i].fd);
...
_recv_del_fd(thread_t *info)
{
	...
	for (i = 0; i < size; i++) {
		DBG("id %u fd %d remove from recv thread\n", 
		    pairs[i].id, pairs[i].fd);
		...
		if (epoll_ctl(rinfo->epfd, EPOLL_CTL_DEL, pairs[i].fd, NULL)) {
			...
			DBG("id %u delete fd %d from epoll\n", 
			    pairs[i].id, pairs[i].fd);
			s->refcnt--;
		}
...
_recv_pkt(thread_t *info, u_int32_t id, int fd)
{
	...
	DBG("id %u fd %u recved %d bytes\n", id, fd, n);

	/* check session and recv result */
 ...
 */
int recv_add_fd(u_int32_t id, int fd)
{
	thread_t *info;
	recv_t *rinfo;
	int size;
 ...
 */
int recv_del_fd(u_int32_t id, int fd)
{
	thread_t *info;
	recv_t *rinfo;
	int size;
	...
	if (fd < 0 || id < 1 || id > g_proxy.capacity)
		return -1;
