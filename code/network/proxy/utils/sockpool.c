...
#define _SKPOOL_BIT_USED	0
#define _SKPOOL_USED		(1 << _SKP_BIT_USED)

#define _SKPOOL_IS_USED(flags)	(flags &  _SKPOOL_USED)
#define _SKPOOL_SET_USED(flags)	(flags |= _SKPOOL_USED)
#define _SKPOOL_CLR_USED(flags)	(flags &= ~_SKPOOL_USED)

typedef struct _skpool_item {
	struct _skpool_item	*prev;
	struct _skpool_item	*next;
	int			fd;
	u_int32_t		nused;
	u_int16_t		flags;
	ip_port_t		addr;
...
#ifdef _SKPOOL_DBG
#define _SKPOOL_ERR(fmt, args...)	\
	fprintf(stderr, "skpool:%s:%d: "fmt,	\
 ...
 */
static u_int8_t 
_skpool_keygen(ip_port_t *addr)
{
	u_int32_t key;

	key = ip_addr_hash(addr->addr);
