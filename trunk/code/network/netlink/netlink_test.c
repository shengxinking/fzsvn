...
_usage(void)
{
	printf("netlink_test <device|neigh|addr|route> <options>\n");
	printf("\tdevice\tdevice function\n");
...
_addr_func(void)
{
	...
	case _NL_ADD:
		...
		ret = nl_add_addr(_g_iif, _g_family, &(ipmask._addr), ipmask.cidr);
	...
	case _NL_FLUSH:
		ret = nl_flush_addr(_g_iif, _g_family);
		break;

	case _NL_LIST:
		ret = nl_list_addr(_g_iif, _g_family, _addr_print, NULL);
		break;
	}
