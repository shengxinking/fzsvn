/**
 *	file	ipfrag.c
 *	brief	Declare function to do IP frag and defrag.
 *
 *	date	2007-07-19
 */
...
//extern SFPERF sfPerf; wxh


/*  P R O T O T Y P E S  ********************************************/
static void Frag3ParseGlobalArgs(char *);
void Frag3ParseArgs(char *, Frag3Context *);
...
static void PrintFragTracker(FragTracker *ft)
{
    ...
    if(ft)
    {
        ...
        printf("        ttl: %d\n", ft->ttl);
        printf("    alerted: %d\n", ft->alerted);
        printf(" frag_flags: 0x%X\n", ft->frag_flags);
        printf(" frag_bytes: %d\n", ft->frag_bytes);
        printf("  calc_size: %d\n", ft->calculated_size);
        printf("  frag_pkts: %d\n", ft->frag_pkts);
        printf("  frag_time: %lu %lu\n", ft->frag_time.tv_sec, 
                ft->frag_time.tv_usec);
        printf("   fraglist: %p\n", ft->fraglist);
        printf("    fl_tail: %p\n", ft->fraglist_tail);
        printf("fraglst cnt: %d\n", ft->fraglist_count);
    }
}
...
{
    printf("Frag3 global config:\n");
    printf("    Max frags: %d\n", global_config.max_frags);
    if(!global_config.use_prealloc)
        printf("    Fragment memory cap: %lu bytes\n", 
                (unsigned long)global_config.memcap);
    else
        printf("    Preallocated frag nodes: %lu\n", 
 ...
 * @return none
 */
void Frag3PrintEngineConfig(Frag3Context *context)
{
    printf("Frag3 engine config:\n");
    printf("    Target-based policy: %s\n", 
            frag_policy_names[context->frag_policy]);
    printf("    Fragment timeout: %d seconds\n", 
            context->frag_timeout);
    printf("    Fragment min_ttl:   %d\n", context->min_ttl);
    printf("    Fragment ttl_limit (not used): %d\n", context->ttl_limit);
    printf("    Fragment Problems: %X\n", context->frag3_alerts);
...
static inline void EventAnomIpOpts(Frag3Context *context)
{
    if(!(context->frag3_alerts & FRAG3_DETECT_ANOMALIES))
        return;
#if 0
    SnortEventqAdd(GENERATOR_SPP_FRAG3,     
            FRAG3_IPOPTIONS,         /* SID */
            1,                       /* rev */
...
Frag3Context *frag_context;      /* context pointer */ 
/**
...
void Frag3Init(char *args)
{
     ...
     */
   // Frag3ParseArgs(args, frag_context);
    ...
    if (!frag3_registered)
    {
        pfn = AddFuncToPreprocList(Frag3Defrag, PRIORITY_NETWORK, PP_FRAG3); //wxh
        if (pfn)
        {
            frag3_registered = 1;
            pfn->context = NULL;
     ...
     */
    //Frag3PrintEngineConfig(frag_context);
 ...
 * @return none
 */
void Frag3ParseArgs(char *args, Frag3Context *context)
{
    char **toks;
    char *index;
    int num_toks;
 ...
 *
 * @return 0 if defrag success, -1 on error.
 */
int 
Frag3Defrag(Packet *p, void *context)
{
    ...
    for (engineIndex = 0; engineIndex < numFrag3Contexts; engineIndex++)
    {
        ...
        {
            //DEBUG_WRAP(DebugMessage(DEBUG_FRAG, "[FRAG3] Found engine context in IpAddrSet\n"););
            break;
        }
        else
        {
    ...
    f3stats.total++;
    //UpdateIPFragStats(&(sfPerf.sfBase), p->pkth->caplen); wxh
...
#endif
    //ProcessPacket(NULL, defrag_pkt.pkth, defrag_pkt.pkt, ft); wxh
