...
static void _element_start(void *ctx, const xmlChar *name, const xmlChar **attrs)
{
	int i = 0;
...
static void _element_end(void *ctx, const xmlChar *name)
{
	printf("<--- element %s end\n", name);
}

static void _element_error(void *ctx, const char *msg, ...)
{
	va_list ap;
	va_start(ap, msg);
	vprintf(msg, ap);
