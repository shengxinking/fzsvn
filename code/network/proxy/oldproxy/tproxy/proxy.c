...
_proxy_init(proxy_t *py)
{
	...
	py->main_tid = pthread_self();

	/* init proxy lock */
	pthread_mutex_init(&py->lock, NULL);
...
proxy_main(proxy_arg_t *arg)
{
	...
	/* create work threads */
	for (i = 0; i < py->nworker; i++) {
		thread_create(&py->workers[i], i, worker_run);
	}
