...
void Stream4Init(char *args)
{
    ...
    snprintf(logfile, sizeof(logfile), "%s/%s", "./", "session.log");
    ...
    while(i < num_toks)
    {
        ...
        else if(!strcasecmp(stoks[0], "max_sessions"))
        {
            if(isdigit((int)stoks[1][0]))
            {
                ...
                {
                    //LogMessage("WARNING %s(%d) => Ludicrous (<8k) max_sessions " "size, setting to default (%d sessions)\n", file_name, file_line, STREAM4_MAX_SESSIONS);
...
void Stream4InitReassembler(char *args)
{
    ...
    for(i=0;i<65536;i++)
    {
        ...
        {
            //sfsnprintfappend(buf, 1024, "%d ", i); wxh
            j++;
        }
    ...
    LogMessage("%s\n", buf);
    memset(buf, 0, 1024 + 1);
    snprintf(buf, sizeof(buf), "    Emergency Ports: "); 
    j=0;
    ...
    for(i=0;i<65536;i++)
    {
        ...
        {
            //sfsnprintfappend(buf, STD_BUF, "%d ", i); wxh
            j++;
        }
...
void PurgeFlushStream(Session *ssn, Stream *s)
{
    ...
    if (!s4_shutdown)
    {
         ...
         */
        //memcpy(&decoder_flags, &pv.decoder_flags, sizeof(DecoderFlags)); wxh
...
/*void DecodeIptablesPkt(Packet *, const struct pcap_pkthdr *, const u_int8_t *);
	    ...
	    grinder = DecodeIptablesPkt;
            (*grinder)(&p, (struct pcap_pkthdr *)&pkth, pkt); wxwxhh*/
            p.ssnptr = ssn;
            p.streamptr = s;
    ...
    if (!s4_shutdown)
    {
        ...
        //memcpy(&pv.decoder_flags, &decoder_flags, sizeof(DecoderFlags)); wxh
    }
}
...
void Stream4ShutdownFunction(int signal, void *foo)
{
     ...
     */
    //memcpy(&decoder_flags, &pv.decoder_flags, sizeof(DecoderFlags)); wxh
    //memset(&pv.decoder_flags, 0, sizeof(DecoderFlags)); wxh
    s4_shutdown = 1;
    ...
    while (right && !done && SEQ_LT(right->seq_num, seq_end))
    {
        ...
        else
        {
            ...
            if ((right->seq_num == seq) && (right->cksum == p->tcph->th_sum))
            {
                ...
                /* Still want to add this number of bytes to totals */
                //SetPreprocBit(p, PP_PERFMONITOR); wxh
